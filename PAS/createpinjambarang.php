								...
								<div class="form-group">
									...
									<select class="form-control" id="id_barang" onchange="changeValue(this.value)" name="id_barang" required="">
										...
										while ($row = mysqli_fetch_array($query)) {
											echo '<option value="' . $row['id_barang'] . '">' . $row['deskripsi'] . '</option>';
											$stok .= "stok['" . $row['id_barang'] . "'] = {stok:'" . addslashes($row['stok']) . "'};\n";
											$deskripsi .= "deskripsi['" . $row['id_barang'] . "'] = {deskripsi:'" . addslashes($row['nama_barang']) . "'};\n";
...
if (isset($_POST['simpan'])) {
	...
	$selSto = mysqli_query($conn, "SELECT * FROM barang WHERE id_barang='$id_barang'");
